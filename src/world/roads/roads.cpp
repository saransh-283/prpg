        ...
        for (int gy = gy_min; gy <= gy_max; ++gy) {
            ...
            for (int j = 0; j < worm_length; ++j) {
                ...
                if (phaseRemaining <= 0) {
                    ...
                    int64_t salt = (int64_t)seed + (int64_t)j + (inBend ? 5100 : 7100);
                    ...
                    if (inBend) {
                        ...
                        if (std::abs(d) < deg2rad(2.0f)) {
                            ...
                            float forced = std::min(maxTurn, deg2rad(12.0f));
...
std::vector<std::vector<int>> generate_roads_grid(const std::vector<std::vector<int>>& input_grid,
    ...
    // To keep roads continuous across chunk borders, sample seed cells over a
    // one-chunk margin so any road that starts in a neighbor chunk and crosses
    // into this chunk will be regenerated consistently here.
    const int cell_spacing = std::max(16, padding / 2);
    ...
    const double spawnProb = (cellCount > 0.0 && num_roads > 0) ? std::min(1.0, std::max(0.0, (double)num_roads / cellCount)) : 0.0;
